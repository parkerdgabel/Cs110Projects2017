...
class DrawingPanel:
    ...
    def __adjust_coordinates(*coords):
        ...
        return tuple([coord + 0 for coord in coords_to_use])
...
'''
    ...
    def draw_polyline(self, *coords, **options):
        '''
        Draws a group of lines between the given coordinates passed as parameters or as a tuple.
        ...
        if len(coords) == 0:
            return
        if isinstance(coords[0], list) or isinstance(coords[0], tuple):   # allow passing list or tuple
            coords = coords[0]
